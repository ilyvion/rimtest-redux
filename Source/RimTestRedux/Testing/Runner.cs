...
using static RimTestRedux.Testing.Validator;
    ...
    public static class Runner
    {
        ...
        public static void RunTestSuite(Type testSuite)
        {
            ...
            if (tally[TestStatus.ERROR] != 0)
            {
                ...
                SetTestSuiteError(
                    testSuite,
                    new Exception($"{tally[TestStatus.ERROR]} test failed")
                );
            }
            else if (tally[TestStatus.SKIP] != 0)
            {
                ...
                SetTestSuiteError(
                    testSuite,
                    new Exception($"{tally[TestStatus.SKIP]} test skipped")
                );
            }
            else if (tally[TestStatus.UNKNOWN] != 0)
            {
                ...
                SetTestSuiteError(
                    testSuite,
                    new Exception($"{tally[TestStatus.UNKNOWN]} test not run yet")
                );
            }
            else
            {
        ...
        public static void RunAssembly(Assembly asm)
        {
            ...
            if (tally[TestSuiteStatus.ERROR] != 0)
            {
                ...
                SetAssemblyError(
                    asm,
                    new Exception($"{tally[TestSuiteStatus.ERROR]} test suite failed")
                );
            }
            else if (tally[TestSuiteStatus.SKIP] != 0)
            {
                ...
                SetAssemblyError(
                    asm,
                    new Exception($"{tally[TestSuiteStatus.SKIP]} test suite skipped")
                );
            }
            else if (tally[TestSuiteStatus.WARNING] != 0)
            {
                ...
                SetAssemblyError(
                    asm,
                    new Exception($"{tally[TestSuiteStatus.WARNING]} test suite have warnings")
                );
            }
            else if (tally[TestSuiteStatus.UNKNOWN] != 0)
            {
                ...
                SetAssemblyError(
                    asm,
                    new Exception($"{tally[TestSuiteStatus.UNKNOWN]} test suite not run yet")
                );
            }
            else
            {
