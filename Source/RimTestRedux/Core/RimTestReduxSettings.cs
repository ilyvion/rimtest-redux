...
using static RimTestRedux.Testing.AssemblyStatusExtension;
using static RimTestRedux.Testing.TestExplorer;
using static RimTestRedux.Testing.TestStatusExtension;
...
namespace RimTestRedux.Core
{
    ...
    /// </summary>
    public class RimTestReduxSettings : ModSettings
    {
        /// <summary>
        ...
        private void DrawControls(Listing_Standard bar)
        {
            ...
            Rect statusAsmLabelRect = rowAsm
                .RightPartPixels(rowAsm.width - asmExpandRect.xMax - WIDTH_CONTROLS_SECTION_GAP)
                .LeftPartPixels(WIDTH_CONTROLS_VIEW_LABEL);
            Rect statusAsmControllerRect = rowAsm.RightPartPixels(
                rowAsm.width - statusAsmLabelRect.xMax
            );

            Rect statusAsmFailRect = statusAsmControllerRect.LeftPartPixels(
                WIDTH_CONTROLS_VIEW_CONTROL
            );
            Rect statusAsmWarnRect = statusAsmControllerRect
                .RightPartPixels(
                    statusAsmControllerRect.width
                        - statusAsmFailRect.width
                        - WIDTH_CONTROLS_CONTROL_GAP
                )
                .LeftPartPixels(WIDTH_CONTROLS_VIEW_CONTROL);
            Rect statusAsmSkipRect = statusAsmControllerRect
                .RightPartPixels(
                    statusAsmControllerRect.width
                ...
                .LeftPartPixels(WIDTH_CONTROLS_VIEW_CONTROL);
            Rect statusAsmUnknownRect = statusAsmControllerRect
                .RightPartPixels(
                    statusAsmControllerRect.width
                ...
                .LeftPartPixels(WIDTH_CONTROLS_VIEW_CONTROL);
            Rect statusAsmPassRect = statusAsmControllerRect
                .RightPartPixels(
                    statusAsmControllerRect.width
            ...
            Widgets.CheckboxLabeled(
                statusAsmFailRect,
                $"{StatusExplorer.GetAssemblyStatusCount(AssemblyStatus.ERROR)} {StatusSymbol(AssemblyStatus.ERROR)}",
                ref FilteredExplorer.failEnabledAsm
            );

            Widgets.CheckboxLabeled(
                statusAsmWarnRect,
                $"{StatusExplorer.GetAssemblyStatusCount(AssemblyStatus.WARNING)} {StatusSymbol(AssemblyStatus.WARNING)}",
                ref FilteredExplorer.warningEnabledAsm
            );

            Widgets.CheckboxLabeled(
                statusAsmUnknownRect,
                $"{StatusExplorer.GetAssemblyStatusCount(AssemblyStatus.UNKNOWN)} {StatusSymbol(AssemblyStatus.UNKNOWN)}",
                ref FilteredExplorer.unknownEnabledAsm
            );

            Widgets.CheckboxLabeled(
                statusAsmPassRect,
                $"{StatusExplorer.GetAssemblyStatusCount(AssemblyStatus.PASS)} {StatusSymbol(AssemblyStatus.PASS)}",
            ...
            Rect statusTSLabelRect = rowTS
                .RightPartPixels(rowTS.width - tsExpandRect.xMax - WIDTH_CONTROLS_SECTION_GAP)
                .LeftPartPixels(WIDTH_CONTROLS_VIEW_LABEL);
            Rect statusTSControllerRect = rowTS.RightPartPixels(
                rowTS.width - statusTSLabelRect.xMax
            );

            Rect statusTSFailRect = statusTSControllerRect.LeftPartPixels(
                WIDTH_CONTROLS_VIEW_CONTROL
            );
            Rect statusTSWarnRect = statusTSControllerRect
                .RightPartPixels(
                    statusTSControllerRect.width
                        - statusTSFailRect.width
                        - WIDTH_CONTROLS_CONTROL_GAP
                )
                .LeftPartPixels(WIDTH_CONTROLS_VIEW_CONTROL);
            Rect statusTSSkipRect = statusTSControllerRect
                .RightPartPixels(
                    statusTSControllerRect.width
                ...
                .LeftPartPixels(WIDTH_CONTROLS_VIEW_CONTROL);
            Rect statusTSUnknownRect = statusTSControllerRect
                .RightPartPixels(
                    statusTSControllerRect.width
                ...
                .LeftPartPixels(WIDTH_CONTROLS_VIEW_CONTROL);
            Rect statusTSPassRect = statusTSControllerRect
                .RightPartPixels(
                    statusTSControllerRect.width
            ...
            Widgets.CheckboxLabeled(
                statusTSFailRect,
                $"{StatusExplorer.GetTestSuiteStatusCount(TestSuiteStatus.ERROR)} {StatusSymbol(TestSuiteStatus.ERROR)}",
                ref FilteredExplorer.failEnabledTS
            );

            Widgets.CheckboxLabeled(
                statusTSWarnRect,
                $"{StatusExplorer.GetTestSuiteStatusCount(TestSuiteStatus.WARNING)} {StatusSymbol(TestSuiteStatus.WARNING)}",
                ref FilteredExplorer.warningEnabledTS
            );

            Widgets.CheckboxLabeled(
                statusTSSkipRect,
                $"{StatusExplorer.GetTestSuiteStatusCount(TestSuiteStatus.SKIP)} {StatusSymbol(TestSuiteStatus.SKIP)}",
                ref FilteredExplorer.skipEnabledTS
            );

            Widgets.CheckboxLabeled(
                statusTSUnknownRect,
                $"{StatusExplorer.GetTestSuiteStatusCount(TestSuiteStatus.UNKNOWN)} {StatusSymbol(TestSuiteStatus.UNKNOWN)}",
                ref FilteredExplorer.unknownEnabledTS
            );

            Widgets.CheckboxLabeled(
                statusTSPassRect,
                $"{StatusExplorer.GetTestSuiteStatusCount(TestSuiteStatus.PASS)} {StatusSymbol(TestSuiteStatus.PASS)}",
            ...
            Rect tRunAllRect = rowT.RightPartPixels(rowT.width - tLabelRect.xMax)
                .LeftPartPixels(WIDTH_CONTROLS_TS_COLLAPSE);
            Rect tLogAllRect = rowT.RightPartPixels(rowT.width - tRunAllRect.xMax)
                .LeftPartPixels(WIDTH_CONTROLS_TS_EXPAND);
            ...
            Rect statusTLabelRect = rowT.RightPartPixels(
            ...
            Rect statusTFailRect = statusTControllerRect.LeftPartPixels(
                WIDTH_CONTROLS_VIEW_CONTROL
            );
            Rect statusTSkipRect = statusTControllerRect
                .RightPartPixels(
                ...
                .LeftPartPixels(WIDTH_CONTROLS_VIEW_CONTROL);
            Rect statusTUnknownRect = statusTControllerRect
                .RightPartPixels(
                    statusTControllerRect.width
                ...
                .LeftPartPixels(WIDTH_CONTROLS_VIEW_CONTROL);
            Rect statusTPassRect = statusTControllerRect
                .RightPartPixels(
                    statusTControllerRect.width
            ...
            Widgets.CheckboxLabeled(
                statusTFailRect,
                $"{StatusExplorer.GetTestStatusCount(TestStatus.ERROR)} {StatusSymbol(TestStatus.ERROR)}",
                ref FilteredExplorer.failEnabledT
            );

            Widgets.CheckboxLabeled(
                statusTSkipRect,
                $"{StatusExplorer.GetTestStatusCount(TestStatus.SKIP)} {StatusSymbol(TestStatus.SKIP)}",
                ref FilteredExplorer.skipEnabledT
            );

            Widgets.CheckboxLabeled(
                statusTUnknownRect,
                $"{StatusExplorer.GetTestStatusCount(TestStatus.UNKNOWN)} {StatusSymbol(TestStatus.UNKNOWN)}",
                ref FilteredExplorer.unknownEnabledT
            );

            Widgets.CheckboxLabeled(
                statusTPassRect,
                $"{StatusExplorer.GetTestStatusCount(TestStatus.PASS)} {StatusSymbol(TestStatus.PASS)}",
            ...
            Rect searchControlRect = rowSearch.RightPartPixels(
                rowSearch.width - searchLabelRect.xMax
            );
        ...
        {
            if (!assemblyVisibility.ContainsKey(asm))
                assemblyVisibility[asm] = true;
        ...
        private void DrawAssemblyLine(Listing_Standard listing, Assembly asm)
        {
            ...
            Rect timeRect = row.RightPartPixels(row.width - runBtnRect.xMax)
                .LeftPartPixels(WIDTH_TIME_LABEL);
            Rect errorRect = row.RightPartPixels(
                    row.width - (timeRect.xMax + WIDTH_LEVEL_INDENT * 0)
                )
                .LeftPartPixels(WIDTH_ERROR_LEVEL);
            Rect toggleLabelRect = row.RightPartPixels(row.width - errorRect.xMax)
        ...
        {
            if (!testSuiteVisibility.ContainsKey(ts))
                testSuiteVisibility[ts] = true;
        ...
        private void DrawTestSuiteLine(Listing_Standard listing, Type ts)
        {
            ...
            Rect timeRect = row.RightPartPixels(row.width - runBtnRect.xMax)
                .LeftPartPixels(WIDTH_TIME_LABEL);
            Rect errorRect = row.RightPartPixels(
                    row.width - (timeRect.xMax + WIDTH_LEVEL_INDENT * 1)
                )
                .LeftPartPixels(WIDTH_ERROR_LEVEL);
            Rect toggleLabelRect = row.RightPartPixels(row.width - errorRect.xMax)
                .LeftPartPixels(WIDTH_TOGGLE_LABEL);
        ...
        private void DrawTestLine(Listing_Standard listing, MethodInfo test)
        {
            ...
            Rect timeRect = row.RightPartPixels(row.width - runBtnRect.xMax)
                .LeftPartPixels(WIDTH_TIME_LABEL);
            Rect errorRect = row.RightPartPixels(
                    row.width - (timeRect.xMax + WIDTH_LEVEL_INDENT * 2)
                )
                .LeftPartPixels(WIDTH_ERROR_LEVEL);
            Rect toggleLabelRect = row.RightPartPixels(row.width - errorRect.xMax)
                .LeftPartPixels(WIDTH_TOGGLE_LABEL);
